...
from vavilov.models import (
    Accession, Country, Passport, Location, Cvterm, Cv, Taxa, TaxaRelationship,
    Person, Db, Dbxref, AccessionRelationship, AccessionTaxa, AccessionSynonym)
...
SHARED_INITIAL_DATA_TO_LOAD = [
    ('cv', join(SHARED_INITIAL_DATA_DIR, 'vavilov_cv.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_biological_status.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_collecting_source.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_person_types.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_accession_types.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_relationship_types.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_synonym_types.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_taxonomic_ranks.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_assay_props.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_trait_types.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_trait_props.csv')),
    ('cvterm', join(SHARED_INITIAL_DATA_DIR,
                    'vavilov_cvterm_plant_parts.csv')),
...
def load_initial_data_from_dict(spec_to_load):
    for table, fixture in spec_to_load:
        ...
        if table in NO_REF_TABLES:
            cmd = ["manage.py", "add_no_reference_table", fixture, '-t', table]
        else:
...
def add_accessions(fhand, silent=True):
    ...
    for accession_data in csv.DictReader(fhand, dialect=comma_dialect):
        accession_data = {
            ...
            for key, value in accession_data.items()
        }
...
def _add_main_accession(accession_data, silent, view_perm_group):
    ...
    accession = add_accession(
        seed_holder_code, seed_holder_institute, acc_type='internal')
    ...
    taxon = add_taxonomies(
        accession_data['Genus'],
        accession_data['Species'],
        subtaxa=subtaxa_name,
        subtaxa_type=subtaxa_type)
    ...
    if donor_code and donor_institute:
        ...
        is_duplicated_from = Cvterm.objects.get(
            cv__name='relationship_types', name='is_duplicated_from')

        AccessionRelationship.objects.create(
            subject=accession, object=donor_accession, type=is_duplicated_from)
    else:
        donor_accession = None

    # Duplicated accessions
    duplicated_code = accession_data['Code of duplicates in other genebank']
    duplicated_institute = accession_data[
        'Code of the genebank holder of the duplicate']
    if duplicated_code and duplicated_institute:
        ...
        is_duplicated_from = Cvterm.objects.get(
            cv__name='relationship_types', name='is_a_duplicated')
        AccessionRelationship.objects.create(
            subject=accession, object=duplicated_acc, type=is_duplicated_from)
    ...
    if collecting_code and collecting_institute:
        ...
        collecting_type = Cvterm.objects.get(
            cv__name='synonym_types', name='collecting')

        AccessionSynonym.objects.create(
            accession=accession,
            synonym_institute=collecting_inst,
            synonym_code=collecting_code,
            type=collecting_type)
    # passport_data
    local_name = accession_data['Local name'] if accession_data[
        'Local name'] else None
    traditional_location = accession_data[
        'Traditional location'] if accession_data[
            'Traditional location'] else None
    collecting_site = accession_data['Collecting site'] if accession_data[
        'Collecting site'] else None
    collecting_province = accession_data['Province'] if accession_data[
        'Province'] else None
    collecting_region = accession_data['Region'] if accession_data[
        'Region'] else None
    collecting_country = accession_data['Country'] if accession_data[
        'Country'] else None
    latitude = accession_data['Latitude'] if accession_data[
        'Latitude'] else None
    longitude = accession_data['Longitude'] if accession_data[
        'Longitude'] else None
    altitude = accession_data['Altitude'] if accession_data[
        'Altitude'] else None
    biological_status = accession_data[
        'Biological status of accession'] if accession_data[
            'Biological status of accession'] else None
    collecting_source = accession_data['Collecting source'] if accession_data[
        'Collecting source'] else None
    acquisition_date = accession_data['Acquisition date'] if accession_data[
        'Acquisition date'] else None
    collecting_date = accession_data['Collecting date'] if accession_data[
        'Collecting date'] else None

    add_passport_data(
        accession,
        local_name=local_name,
        traditional_location=traditional_location,
        site=collecting_site,
        province=collecting_province,
        region=collecting_region,
        country=collecting_country,
        latitude=latitude,
        longitude=longitude,
        altitude=altitude,
...
def add_passport_data(accession,
                      local_name=None,
                      traditional_location=None,
                      site=None,
                      province=None,
                      region=None,
                      country=None,
    ...
    if country is not None:
        try:
            country = Country.objects.get(
                Q(code2=country) | Q(code3=country) | Q(name=country))
    ...
    if biological_status is not None:
        biological_status = Cvterm.objects.get(
            cv__name='biological_status', name=biological_status)
    if collecting_source is not None:
        collecting_source = Cvterm.objects.get(
            cv__name='collecting_source', name=collecting_source)
...
def add_location(site=None,
                 ...
                 latitude=None,
                 altitude=None):
    if site or province or region or longitude or (
            latitude and longitude) or altitude or country:
...
def _add_taxa(taxa_name, taxa_rank_name, parent_taxa):
    taxo_cv = Cv.objects.get(name='taxonomic_ranks')
    taxa_rank = Cvterm.objects.get_or_create(
        cv=taxo_cv, name=taxa_rank_name)[0]
    is_a = Cvterm.objects.get(cv__name='relationship_types', name='is_a')
    try:
        tr = TaxaRelationship.objects.get(
            taxa_subject__name=taxa_name,
    ...
    if not taxa_in_db:
        ...
        TaxaRelationship.objects.create(
            taxa_subject=taxa, taxa_object=parent_taxa, type=is_a)
    else:
...
def add_accession(accession, institute_name, acc_type=None, make_link=True):
    ...
    if acc_type is not None:
        acc_type = Cvterm.objects.get(
            cv__name='accession_types', name=acc_type)

    institute = Person.objects.get(name=institute_name)
    try:
        accession = Accession.objects.get(
            accession_number=accession, institute=institute)
